...
!**************************************************
      ...
      module genUtils
      use stdParams, only: strL, newl, maxToks, stdout, eps
      implicit none
      ...
      contains
         ...
         !==========================================
         function TO_UPPER(strng) result (res)
         implicit none
         character(len=*), intent(in) :: strng
         character(len=strL) :: res
         ...
         res = trim(strng)
         do i=1, len(trim(strng))
            select case(strng(i:i))
            case("a":"z")
               res(i:i) = ACHAR(IACHAR(strng(i:i))-32)
            end select
         end do
         ...
         !==========================================
         function TO_LOWER(strng) result (res)
         implicit none
         character(len=*), intent(in) :: strng
         character(len=strL) :: res
         ...
         pure function endsWith(a,b) result(c)
         implicit none
...
!        absolute values are calculated and i make sure "a" is bigger
         ...
         !==========================================
         ! Swaps double precision numbers
         ...
         if (c .eq. 'a') then
            do i=1, n
               do j=i+1, n
                  if (u(j) .lt. u(i)) call swap(u(j), u(i))
               end do
            end do
         else
            do i=1, n
               do j=i+1, n
                  if (u(j) .gt. u(i)) call swap(u(j), u(i))
               end do
            end do
         end if
